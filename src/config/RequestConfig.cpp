...
/*   Updated: 2024/06/01 14:08:19 by doduwole         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../inc/AllHeaders.hpp"

RequestConfig::RequestConfig(HttpRequest &request, Listen &host_port, DB &db,
                             Client &client)
    : request_(request), client_(client), host_port_(host_port), db_(db) {
  isLociMatched_ = 0;
  redir_code_ = 0;
  location_cache_ = "";
}
...
RequestConfig::RequestConfig(const RequestConfig &rhs)
    : request_(rhs.request_), targetServer_(rhs.targetServer_),
      client_(rhs.client_), host_port_(rhs.host_port_), db_(rhs.db_),
      modifierType_(rhs.modifierType_), target_(rhs.target_), root_(rhs.root_),
      uri_(rhs.uri_), client_max_body_size_(rhs.client_max_body_size_),
      autoindex_(rhs.autoindex_), indexes_(rhs.indexes_),
      error_codes_(rhs.error_codes_), redirectMap_(rhs.redirectMap_),
      allowed_methods_(rhs.allowed_methods_), serverId(rhs.serverId),
      auth_(rhs.auth_), upload_(rhs.upload_), cgi_(rhs.cgi_),
      locationsMap_(rhs.locationsMap_), isLociMatched_(rhs.isLociMatched_),
      uri_suffix_(rhs.uri_suffix_), redir_code_(rhs.redir_code_) {}

RequestConfig::RequestConfig(const RequestConfig &rhs, HttpRequest &request,
                             Client &client)
    : request_(request), targetServer_(rhs.targetServer_), client_(client),
      host_port_(rhs.host_port_), db_(rhs.db_),
      modifierType_(rhs.modifierType_), target_(rhs.target_), root_(rhs.root_),
      uri_(rhs.uri_), client_max_body_size_(rhs.client_max_body_size_),
      autoindex_(rhs.autoindex_), indexes_(rhs.indexes_),
      error_codes_(rhs.error_codes_), redirectMap_(rhs.redirectMap_),
...
const VecStr &RequestConfig::filterDataByDirectives(
    const std::vector<KeyMapValue> &targetServ, std::string directive,
    std::string location = "") {
...
bool RequestConfig::directiveExists(std::string directive,
                                    std::string location) {
  if (location.empty())
    location = "/";
...
const VecStr &RequestConfig::checkRootDB(std::string directive) {
  ...
  for (it = db_.rootDB.begin(); it != db_.rootDB.end(); ++it) {
    ...
    for (size_t i = 0; i < values.size(); ++i) {
      ...
      const VecStr &dirValue =
          filterDataByDirectives(values, directive, location);
      if (!dirValue.empty())
        return valueVector;
    }
  }
...
const VecStr &RequestConfig::cascadeFilter(std::string directive,
                                           std::string location = "") {
  const VecStr &dirValue =
      filterDataByDirectives(targetServer_, directive, location);
  if (!dirValue.empty())
    return dirValue;

  if (dirValue.empty() && !location.empty()) {
    const VecStr &filteredValue =
        filterDataByDirectives(targetServer_, directive, "");
...
int &RequestConfig::getRedirCode() { return redir_code_; }

void RequestConfig::setRedirCode(int code) { redir_code_ = code; }
...
findCaseInsensitive(const std::map<std::string, int> &myMap,
  ...
  for (std::map<std::string, int>::const_iterator it = myMap.begin();
    ...
    std::transform(it->first.begin(), it->first.end(),
                   std::back_inserter(lowerFirst), ::tolower);
...
std::string RequestConfig::findLongestMatch() {
  ...
  std::map<std::string, int>::const_iterator locationsMap =
      getLocationsMap().begin();
  while (locationsMap != getLocationsMap().end()) {
    if (target.find(locationsMap->first) == 0 &&
        locationsMap->first.length() > longestMatch.length())
...
void RequestConfig::setLociMatched(int status) { isLociMatched_ = status; }

int RequestConfig::getLociMatched() { return isLociMatched_; }
...
void RequestConfig::redirectLocation(std::string target) { target_ = target; }
...
void RequestConfig::setClientMaxBodySize(const VecStr size) {
  size_t val =
      size.empty() ? 209715200 : (std::istringstream(size[0]) >> val, val);
  client_max_body_size_ = val;
}

void RequestConfig::setUpload(const VecStr &upload) { upload_ = upload[0]; }

void RequestConfig::setCgi(bool &val) { client_.setCgi(val); }

bool RequestConfig::get_Cgi() { return client_.getCgi(); }
...
void RequestConfig::setAutoIndex(bool autoindex) { autoindex_ = autoindex; }

void RequestConfig::setIndexes(const VecStr &indexes) { indexes_ = indexes; }

void RequestConfig::setMethods(const VecStr &methods) {
  allowed_methods_ =
      methods.empty() ? cascadeFilter("limit_except", target_) : methods;
}

void RequestConfig::setCgi(const VecStr &cgi) { cgi_ = cgi; }

void RequestConfig::assignCodes(const std::string &codes,
...
void RequestConfig::setMap(const VecStr &vec,
                           std::map<int, std::string> &resultMap,
                           std::string &codes) {
  if (!vec.empty()) {
...
std::string RequestConfig::getUriSuffix() { return request_.getUriSuffix(); }
...
std::string &RequestConfig::getTarget() { return target_; }

std::string &RequestConfig::getRequestTarget() { return request_.getURI(); }

std::string &RequestConfig::getQuery() { return request_.getQuery(); }

std::string &RequestConfig::getFragment() { return request_.getFragment(); }

std::string &RequestConfig::getMethod() { return request_.getMethod(); }

std::string &RequestConfig::getHost() { return host_port_.ip_; }

uint32_t &RequestConfig::getPort() { return host_port_.port_; }

size_t RequestConfig::getContentLength() { return request_.getContentLength(); }

Client &RequestConfig::getClient() { return client_; }

std::string &RequestConfig::getRoot() { return root_; }

std::string &RequestConfig::getUri() { return uri_; }

std::string &RequestConfig::getAuth() { return auth_; }

size_t &RequestConfig::getClientMaxBodySize() { return client_max_body_size_; }

bool RequestConfig::getAutoIndex() { return autoindex_; }

std::vector<std::string> &RequestConfig::getIndexes() { return indexes_; }
...
const std::string &RequestConfig::getBody() const { return request_.getBody(); }
...
std::string &RequestConfig::getProtocol() { return request_.getProtocol(); }

std::string &RequestConfig::getUpload() { return upload_; }

std::vector<std::string> &RequestConfig::getCgi() { return cgi_; }
...
bool RequestConfig::isMethodAccepted(std::string &method) {
  ...
  methodFlag = directiveExists("allow_methods", location_cache_) ||
               directiveExists("limit_except", location_cache_);
  ...
  return (method.empty())
             ? false
             : (std::find(allowed_methods_.begin(), allowed_methods_.end(),
...
void RequestConfig::setClient(Client &client) { client_ = client; }

std::string &RequestConfig::getBody() { return request_.getBody(); }