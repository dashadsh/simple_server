...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/01 10:44:06 by doduwole          #+#    #+#             */
/*   Updated: 2024/06/05 12:43:51 by doduwole         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../inc/ConfigDB.hpp"

ConfigDB::ConfigDB() { counter = 0; }

ConfigDB::~ConfigDB() {}

ConfigDB::ConfigDB(const ConfigDB &rhs)
    : _variablePath(rhs._variablePath), sectionCounts(rhs.sectionCounts),
      _keyValues(rhs._keyValues), groupedServers(rhs.groupedServers),
      groupedRootData(rhs.groupedRootData), counter(rhs.counter) {}
...
std::string ConfigDB::handleKeySection(int &start, int &end,
                                       std::string &line) {
  ...
  if (sectionCounts[currentSection] >= 0) {
    std::stringstream ss;
    ss << "["
       << ((!server) ? sectionCounts[currentSection] : sectionCounts["server"])
       << "]";
...
void ConfigDB::printKeyValue() {
  ...
  for (MapIterator it1 = _keyValues.begin(); it1 != _keyValues.end(); ++it1) {
    ...
    for (VecStr::const_iterator it2 = values.begin(); it2 != values.end();
         ++it2)
...
void ConfigDB::fillMap(std::string value, std::string key,
                       std::string currentSection,
  ...
  while (spltValIt != splitedValue.end()) {
    if (!currentSection.empty()) {
      _keyValues[KeyWithoutLastSection + currentSection + "." + key].push_back(
          *spltValIt);
    } else {
...
void ConfigDB::splitDirectiveAndValue(std::string currentSection,
...
ConfigDB::KeyValues ConfigDB::getKeyValue() { return this->_keyValues; }

void ConfigDB::handleRootDB(MapStr keyMap, const std::string &key,
                            const VecStr &values, size_t lhs, size_t rhs) {
...
void ConfigDB::handleServerDB(MapStr keyMap, const std::string &key,
                              const VecStr &values, size_t lhs, size_t rhs) {
  ...
  if (locationStart != std::string::npos) {
    ...
    if (locationEnd != std::string::npos)
      keyMap["location"] =
          key.substr(locationStart + 9, locationEnd - locationStart - 9);
  }
...
void ConfigDB::printChoice(bool allRootData, int rootDataIdx,
                           bool allServersData, int serverDataIdx,
                           bool allConfig) {
  if (allRootData) {
...
ConfigDB::GroupedDBMap ConfigDB::getServers() const { return groupedServers; }
