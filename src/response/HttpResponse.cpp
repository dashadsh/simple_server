...
/*   Updated: 2024/06/01 15:21:37 by doduwole         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../inc/HttpResponse.hpp"

HttpResponse::HttpResponse(RequestConfig &config, int error_code)
    : config_(config), file_(NULL), error_code_(error_code) {
  worker_id_ = 0;
  total_sent_ = 0;
  status_code_ = 0;
  response_ = "";
...
HttpResponse::~HttpResponse() { cleanUp(); }
...
std::pair<std::string, int> HttpResponse::findLocation(std::string target) {
  ...
  for (std::map<std::string, int>::iterator it = locationsMap.begin();
       it != locationsMap.end(); ++it) {
    if (target == it->first)
      return *it;
  }
...
void HttpResponse::build() {
  ...
  file_ = new File();
  bool isBodyLimit =
      config_.getMethod() == "POST" || config_.getMethod() == "PUT";
  ...
  } else if (isBodyLimit && config_.getClientMaxBodySize() > 0 &&
             config_.getBody().length() > config_.getClientMaxBodySize()) {
    status_code_ = 413;
...
void HttpResponse::handlePutPostRequest() {
  ...
  if (!config_.getUpload().empty()) {
    ...
    path = config_.getUri() + "/" + config_.getUpload() + "/" +
           config_.getTarget();
...
void HttpResponse::handleAcceptLanguage(std::vector<std::string> &matches) {
  ...
  if (!config_.getHeader("Accept-Language").empty()) {
    if (localization(matches))
      file_->set_path(
          path.substr(0, path.find_last_of("/") + 1) + matches.front(), true);
  }
}
...
bool HttpResponse::checkAuth() {
  ...
  if (authCredentials.empty())
    return false;
  std::string token =
      b64decode(authCredentials.substr(authCredentials.find(' ') + 1));
...
bool HttpResponse::getRedirect() { return redirect_; }

std::string HttpResponse::redirect_target() { return redirect_target_; }
...
void HttpResponse::createResponse() {
  ...
  std::string status_line =
      "HTTP/1.1 " + ftos(status_code_) + " " + status_code_phrase + "\r\n";
  ...
  std::cout << status_code_ << " " << ((status_code_ < 400) ? BGREEN : BRED)
            << status_code_phrase << RESET << std::endl;

  if (!foundIndex && config_.getAutoIndex())
    std::cout << CURSIVE_GRAY << "Displaying AutoIndex!!!" << RESET
              << std::endl;
}

std::string HttpResponse::getSampleResponse() { return response_; }
...
int HttpResponse::sendResponse(int fd) {
  ...
  int ret = send(fd, fullResponse.c_str() + total_sent_,
                 fullResponse.length() - total_sent_, 0);
  if (ret <= 0) {
...
File *HttpResponse::getFile() { return file_; }
...
void HttpResponse::clearBody() { body_.clear(); }

void HttpResponse::setConfig(RequestConfig &config) { config_ = config; }

std::map<std::string, std::string> &HttpResponse::getHeadersRef() {
  return headers_;
}

bool HttpResponse::getCgiStatus() { return cgi_true_; }

int HttpResponse::getStatus() { return status_code_; }