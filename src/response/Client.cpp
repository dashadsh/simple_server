...
/*   Updated: 2024/06/01 14:09:02 by doduwole         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../inc/AllHeaders.hpp"

Client::Client(DB &db, Listen &host_port, HttpRequest &req_,
               size_t targetServerIdx, int status)
    : request_(&req_), host_port_(host_port), config_(NULL), response_(NULL),
      db_(db), serverId_(targetServerIdx), statusCode_(status) {
  setupConfig();
  is_cgi_ = false;
  if (!request_->getMethod().empty() || !request_->getTarget().empty()) {
    ...
    std::cout << request_->getMethod() << RESET << " " << request_->getTarget()
              << " " << request_->getProtocol() << std::endl;
  }
}
...
Client::Client(const Client &rhs)
    : host_port_(rhs.host_port_), db_(rhs.db_), serverId_(rhs.serverId_),
...
void Client::setCgi(bool &val) { is_cgi_ = val; }
...
bool Client::getCgi() { return is_cgi_; }
...
void Client::printRouting() {
  ...
  switch (statusCode) {
  case 0:
    std::cout << HIBGWHITE << "Rerouting:" << RESET << " "
              << " " << redir;
    break;
  case 301:
  ...
  if (statusCode != 301)
    std::cout << ", " << HIBGGREEN << "Port: " << host_port_.port_ << RESET
              << std::endl;
}
...
RequestConfig *Client::getConfig() { return config_; }

RequestConfig &Client::getConfigRef() { return *config_; }

HttpResponse &Client::getResponseRef() { return *response_; }

HttpRequest &Client::getRequestRef() { return *request_; }

void Client::setConfig(RequestConfig &config) { config_ = &config; }

void Client::setResponse(HttpResponse &response) { response_ = &response; }