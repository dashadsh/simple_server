...
File::File(std::string path) : fd_(0) { set_path(path); }

File::~File() { closeFile(); }

File::File(const File &rhs)
    : fd_(rhs.fd_), mime_ext_(rhs.mime_ext_), file_name_(rhs.file_name_),
      file_name_full_(rhs.file_name_full_), matches_(rhs.matches_),
      path_(rhs.path_) {}
...
void File::parseExt() {
  ...
  if (lastDotPos != std::string::npos && lastDotPos != 0) {
    ...
    std::transform(mime_ext_.begin(), mime_ext_.end(), mime_ext_.begin(),
                   tolower);
  ...
  file_name_ =
      (lastDotPos != std::string::npos) ? file.substr(0, lastDotPos) : file;
}
...
int File::appendFile(const std::string &body) {
  ...
  if (fd_ < 0) {
    std::cerr << "Error opening file for append: " << strerror(errno)
              << std::endl;
    return 400;
  }
...
bool File::checkFileExists(const std::string &filePath) {
  ...
  if (stat(filePath.c_str(), &statbuf) != 0) {
    std::cerr << "Error checking existence for " << filePath << ": "
...
bool File::exists() { return checkFileExists(path_); }

bool File::exists(const std::string &path) { return checkFileExists(path); }
...
bool File::is_directory() {
  ...
  if (stat(path_.c_str(), &statbuf) != 0) {
    std::cerr << "Error getting file info for " << path_ << ": "
              << strerror(errno) << std::endl;
    return false;
  }
...
std::string File::listDir(std::string &target) {
  ...
  body.append("<div style=\"display: grid; grid-template-columns: 1fr 1fr 1fr; "
              "align-items: center; padding: 0px 20px; font-size: 1rem; "
              "font-weight:bold;\">");
...
std::string File::formatListing(const directory_listing &listing,
                                const std::string &basePath) {
  std::string formatted;

  std::string link = removeDupSlashes(basePath + "/" + listing.name_);
  formatted.append("<div style=\"display: grid; grid-template-columns: 1fr 1fr "
                   "1fr; align-items: center; padding: 0px 20px;\">");
  formatted.append("<a style=\"font-weight:bold;\" href=\"" + link + "\">" +
                   listing.name_ + "</a>");
  formatted.append("<span>" + listing.date_ + "</span>");
  formatted.append("<span>" + (listing.is_dir_ ? "-" : ftos(listing.size_)) +
                   "</span>");
...
std::vector<directory_listing>
File::getDirListings(const std::string &dirPath) {
...
directory_listing File::createListing(const std::string &fileName,
...
void File::print_file_info(const std::string &file) const {
  ...
  if (stat(filename.c_str(), &file_stat) == 0) {
    ...
    (S_ISDIR(file_stat.st_mode) ? std::cout << "Type: Directory"
...
std::string File::find_index(std::vector<std::string> &indexes) {
  ...
  if (dir) {
    while ((ent = readdir(dir))) {
      // print_dir_entry(ent);
      if (std::find(indexes.begin(), indexes.end(), ent->d_name) !=
          indexes.end()) {
        std::string ret = "/";
        ret += ent->d_name;
        // print_file_info(ret);
    ...
    std::cout << "opendir : " << strerror(errno) << " of " << path_
              << std::endl;
...
void File::findMatchingFiles() {
  ...
  if (dir) {
    ...
    std::cerr << "opendir : " << strerror(errno) << " of " << path_
              << std::endl;
  }
}

std::vector<std::string> &File::getMatches() { return matches_; }
...
int &File::getFd() { return fd_; }

std::string &File::getMimeExt() { return mime_ext_; }

std::string &File::getFilePath() { return path_; }
...
bool File::updateFile(const std::string &content) {
  ...
  fd_ = open(path_.c_str(), flags, 0644);
  if (fd_ < 0) {
    std::cerr << "Error opening file for update: " << strerror(errno)
              << std::endl;
    return false;
  }
  ssize_t bytes_written = write(fd_, content.c_str(), content.length());
