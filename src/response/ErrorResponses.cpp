...
/*   Updated: 2024/06/05 12:07:06 by doduwole         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void HttpResponse::redirectToErrorPage(const std::string &target,
                                       int status_code) {
...
std::string HttpResponse::buildDefaultErrorPage(int status_code) {
  ...
  errorPage.append("<title>" + ftos(status_code) + " " +
                   file_->getStatusCode(status_code) + "</title>\r\n");
  errorPage.append("<meta charset=\"utf-8\">\r\n");
  errorPage.append("<meta name=\"viewport\" content=\"width=device-width, "
                   "initial-scale=1.0\">\r\n");
  errorPage.append("<meta name=\"description\" content=\"" +
                   file_->getStatusCode(status_code) + "\">\r\n");
  errorPage.append("<meta name=\"author\" content=\"Your Website\">\r\n");
  errorPage.append("</head>\r\n");
  errorPage.append("<body>\r\n");
  errorPage.append("<center><h1>" + ftos(status_code) + " " +
                   file_->getStatusCode(status_code) + "</h1></center>\r\n");
