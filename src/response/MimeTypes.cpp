...
std::string MimeTypes::getExt(const std::string &type) {
  for (std::map<std::string, std::string>::iterator it = mimeMap.begin();
       it != mimeMap.end(); ++it) {
    if (it->second == type)
      return it->first;
  }
...
std::map<std::string, std::string> MimeTypes::getMap() { return mimeMap; }

MimeTypes::~MimeTypes() { mimeMap.clear(); }