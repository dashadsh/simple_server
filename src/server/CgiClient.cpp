...
CgiClient::CgiClient(Client client, int epoll_fd)
    : client_(client), cgiHeadersParsed_(false), cgiRead_(false),
      cgi_bytes_read_(0), cgi_times_read_(0), status_code_(0),
      epoll_fd_(epoll_fd) {
...
void CgiClient::toCgi(CgiHandle &cgi, std::string &req_body) {
  if (cgi.getContentLength() > 0) {
    int bytesWritten =
        write(cgi.getPipeIn(), req_body.c_str(), req_body.length());
    if (bytesWritten >= 0) {
...
void CgiClient::setCgiPipe(CgiHandle &cgi) {
  int flags;
  if ((flags = fcntl(cgi.getPipeOut(), F_GETFL, 0)) == -1 ||
      fcntl(cgi.getPipeOut(), F_SETFL, flags | O_NONBLOCK) == -1) {
...
HttpResponse &CgiClient::getResponse() { return client_.getResponseRef(); }

int CgiClient::getStatusCode() { return status_code_; }
...
int CgiClient::getPipeOut(void) { return (cgi_->getPipeOut()); }
