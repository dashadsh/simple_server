...
epoll_fd_(epoll_fd)
{
	...
	// objects are lost here for some reason -> savig addresses here
    response_ = &client_.getResponseRef(); // http response
    config_ = &client_.getConfigRef(); // request config
	cgiHeadersMap_ = &response_->getHeadersRef(); // response headers
	
	response_->setConfig(*config_); //just i case assigning config to response
	config_->setClient(client_); // config gets client attached (bc doesnt have FD itself)
    ...
    pid_ = cgi_->getPid(); // get PID of child process

	body_ = &response_->getBody(); // same refs as above
	req_body_ = &config_->getBody(); // same refs as above

	setCgiPipe(*cgi_); // set pipe to non-blocking with fnctl
	toCgi(*cgi_, *req_body_); // send request body to CGI -> delete REDUNDANT STUFF HERE?? - no chunks
}
...
// send request body to CGI -> delete REDUNDANT STUFF HERE?? - no chunks
// this function will be used only once IN CONSTRUCTOR
void CgiClient::toCgi(CgiHandle &cgi, std::string &req_body)
{
	if (cgi.getContentLength() > 0)
	{
		...
		if (bytesWritten >= 0)
		{	// deduct content length - redundant stuff -> will be sent all at the same
...
// set pipe to non-blocking with fnctl
void CgiClient::setCgiPipe(CgiHandle &cgi)
{
	int flags;
	// fnctl to set pipe to non-blocking
