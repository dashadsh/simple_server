...
/*   Updated: 2024/06/07 23:17:24 by drey             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Servers::Servers(ConfigDB &configDB)
    : _epoll_fds(-1), _server_fds(), _domain_to_server(), _ip_to_server(),
      _keyValues(), server_index(), server_fd_to_index(), client_to_server(),
      _client_amount(0), _client_data(), _cgi_clients_childfd(), _client_time(),
      configDB_(configDB) {
  servers = this;
  stop_fd = -1;
...
Servers::~Servers() {
  for (std::map<int, CgiClient *>::iterator it = _cgi_clients.begin();
       it != _cgi_clients.end(); it++) {
    if (it->second != NULL)
      delete it->second;
  }
  for (std::map<int, int>::iterator it = client_to_server.begin();
       it != client_to_server.end(); it++) {
    if (close(it->first) == -1)
...
Servers::Servers(const Servers &rhs)
    : _epoll_fds(rhs._epoll_fds), _server_fds(rhs._server_fds),
      ...
      _ip_to_server(rhs._ip_to_server), _keyValues(rhs._keyValues),
      server_index(rhs.server_index),
      server_fd_to_index(rhs.server_fd_to_index),
      _client_amount(rhs._client_amount), configDB_(rhs.configDB_) {}
...
int Servers::bindSocket(std::string s_port) {
  ...
  if (bind(_server_fds.back(), (struct sockaddr *)&address, sizeof(address)) ==
      -1) {
...
void printRow(int width, int serverNum, int ipWidth, std::string ip) {
  std::cout << "| " << std::left << std::setw(width) << serverNum << " | "
            << std::setw(ipWidth) << ip << " |" << std::endl;
}

void Servers::createServers() {
  std::cout << "|" << std::string(7, ' ') << CURSIVE_GRAY
            << " Creating servers..." << RESET << std::string(9, ' ') << "|"
            << std::endl;
  ...
  std::cout << "| " << std::left << std::setw(serverNumberWidth - 2)
            << "Server ID"
            << " | " << std::setw(portWidth - 2) << "Port"
            << " |" << std::endl;
  std::cout << "+---------------+--------------------+" << std::endl;

  for (std::vector<std::string>::iterator it2 = ports.begin();
       it2 != ports.end(); it2++) {
    if (!checkSocket(*it2)) {
      if (createSocket()) {
        if (!bindSocket(*it2) || !listenSocket() ||
            !setNonBlocking(_server_fds.back()) ||
            !combineFds(_server_fds.back()))
          _server_fds.pop_back();
        else {
          assignDomain(*it2, _server_fds.back());
          printRow(serverNumberWidth - 2, _server_fds.back() - 3, portWidth - 2,
                   _ip_to_server[_server_fds.back()]);
        }
      }
    }
  }
...
Listen getTargetIpAndPort(std::string requestedUrl) {
  ...
  if (pos == std::string::npos)
    return (std::cout << "Warning: default host and port used 127.0.0.1:80"
                      << std::endl,
            listen);
...
void Servers::handleIncomingData(int client_fd) {
  ...
  if (!handleResponse(reqStatus, server_fd, client_fd,
                      _client_data.find(client_fd)->second))
    return;
...
void Servers::initEvents() {
  while (1) {
    try {
      ...
      std::vector<struct epoll_event> events(_server_fds.size() +
                                             _client_amount);
      int n = epoll_wait(this->_epoll_fds, events.data(), events.size(),
                         1000); // Use data() to get the underlying array
      if (n == -1) {
      ...
      for (int i = 0; i < n; i++) {
        bool server = false;
        for (std::vector<int>::iterator it2 = _server_fds.begin();
             it2 != _server_fds.end(); ++it2) {
    ...
    if (it_domain->first.find("server_name") != std::string::npos) {
      ...
      if (it_domain_listen == config.end()) {
        for (std::vector<std::string>::iterator it = it_domain->second.begin();
             it != it_domain->second.end(); it++) {
          if (std::find(_domain_to_server[server_fd].begin(),
                        _domain_to_server[server_fd].end(),
                        *it) == _domain_to_server[server_fd].end())
      ...
      if (*it2 == port) {
        ...
        if (it_domain != config.end()) {
          for (std::vector<std::string>::iterator it3 =
                   it_domain->second.begin();
               it3 != it_domain->second.end(); it3++) {
            if (std::find(_domain_to_server[server_fd].begin(),
                          _domain_to_server[server_fd].end(),
                          *it3) == _domain_to_server[server_fd].end())
              _domain_to_server[server_fd].push_back(*it3);
          }
        } else {
          if (std::find(_domain_to_server[server_fd].begin(),
                        _domain_to_server[server_fd].end(),
                        "localhost") == _domain_to_server[server_fd].end())
...
int Servers::checkSocketPort(std::string port) {
  for (std::string::iterator it = port.begin(); it != port.end(); it++) {
    if (!std::isdigit(*it) || std::atoi(port.c_str()) < 0 ||
...
size_t Servers::handleResponse(int reqStatus, int server_fd, int new_socket,
                               HttpRequest &parser) {
  std::string response;
  if (reqStatus != 200) {
    ...
    if (client.getResponseRef().getStatus() <= 400 &&
        (client.getCgi() || client.getCgiResponse())) {
...
int Servers::handleIncomingCgi(int child_fd) {
  std::string response;
  int client_fd = -1;
  for (std::map<int, int>::iterator it = _cgi_clients_childfd.begin();
       it != _cgi_clients_childfd.end(); it++) {
  ...
  if (_cgi_clients[client_fd]->getStatusCode() == 200 ||
      _cgi_clients[client_fd]->getStatusCode() == 500) {
...
void Servers::checkClientTimeout() {
  ...
  for (std::map<int, time_t>::iterator it = _client_time.begin();
       it != _client_time.end(); it++) {
    if (_cgi_clients.find(it->first) != _cgi_clients.end()) {
      if (current_time - it->second >= 2) {
        for (std::map<int, int>::iterator it2 = _cgi_clients_childfd.begin();
             it2 != _cgi_clients_childfd.end(); it2++) {
...
void Servers::printData() {
  ...
  for (std::map<int, int>::iterator it = client_to_server.begin();
       it != client_to_server.end(); it++)
    std::cout << "Client: " << it->first << " Server: " << it->second
              << std::endl;
  std::cout << "----------CLIENT-DATA-----------" << std::endl;
  for (std::map<int, HttpRequest>::iterator it = _client_data.begin();
       it != _client_data.end(); it++)
    std::cout << "Client: " << it->first << std::endl;
  std::cout << "----------CGI-CLIENTS-----------" << std::endl;
  for (std::map<int, CgiClient *>::iterator it = _cgi_clients.begin();
       it != _cgi_clients.end(); it++)
    std::cout << "Client: " << it->first
              << " Response: " << it->second->getResponseString() << std::endl;
  std::cout << "--------CGI-CLIENTS-CHILDFD--------" << std::endl;
  for (std::map<int, int>::iterator it = _cgi_clients_childfd.begin();
       it != _cgi_clients_childfd.end(); it++)
    std::cout << "Child: " << it->first << " Client: " << it->second
              << std::endl;
  std::cout << "----------CLIENT-TIME-----------" << std::endl;
  for (std::map<int, time_t>::iterator it = _client_time.begin();
       it != _client_time.end(); it++)
    std::cout << "Client: " << it->first << " Time: " << it->second
              << std::endl;
  std::cout << std::endl;
}
...
void Servers::removeFromEpoll(int fd) {
  if (epoll_ctl(this->_epoll_fds, EPOLL_CTL_DEL, fd, NULL) == -1) {
    std::cerr << "Failed to remove client file descriptor from epoll instance."
              << std::endl;
  }
}