...
CgiHandle::CgiHandle(RequestConfig *config, std::string cgi_ext, int epoll_fd)
    : _config(config), _cgi_path(""), _cgi_pid(-1), _cgi_ext(cgi_ext),
      _exit_status(0), _argv(NULL), _envp(NULL), _path(NULL), pipe_in(),
      pipe_out(), content_length(0), epoll_fd_(epoll_fd) {
  this->initEnv();
  this->execCgi();
}
...
void CgiHandle::initEnv() {
  ...
  this->_env["HTTP_UPGRADE_INSECURE_REQUESTS"] =
      this->_config->getHeader("upgrade-insecure-requests");
  ...
  this->_env["HTTP_ACCEPT_ENCODING"] =
      this->_config->getHeader("accept-encoding");
  this->_env["HTTP_ACCEPT_LANGUAGE"] =
      this->_config->getHeader("accept-language");
  this->_env["HTTP_REFERER"] = "http://" + this->_config->getHost() + ":" +
                               port + "/cgi-bin" + this->_config->getUri();
...
void CgiHandle::execCgi() {
  ...
  if (_cgi_pid < 0) {
    ...
    if (dup2(this->pipe_in[0], STDIN_FILENO) == -1 ||
        dup2(this->pipe_out[1], STDOUT_FILENO) == -1 ||
        (_exit_status = execve(this->_argv[0], this->_argv, this->_envp)) ==
            -1) {
...
int CgiHandle::getPipeIn() { return this->pipe_in[1]; }

int CgiHandle::getPipeOut() { return this->pipe_out[0]; }

int CgiHandle::getExitStatus() { return this->_exit_status; }
int CgiHandle::getContentLength() { return this->content_length; }

int CgiHandle::getPid() { return this->_cgi_pid; }
