...
int HttpRequest::extractRequestLineData(std::string requestLine) {
  ...
  if (methodEnd == std::string::npos || protocolStart == std::string::npos ||
      protocolStart <= uriStart)
    return 400;

  method_ = requestLine.substr(0, methodEnd);
  uri_ = requestLine.substr(uriStart, protocolStart - uriStart);
  (uri_.find("?") != std::string::npos)
      ? setTarget(uri_.substr(0, uri_.find("?")))
      : setTarget(uri_);
...
int HttpRequest::parseMethod() {
  static const std::string validMethodsArray[] = {"GET",    "POST",  "PUT",
                                                  "DELETE", "PATCH", "OPTIONS",
                                                  "HEAD",   "TRACE", "CONNECT"};
  static const std::set<std::string> validMethods(
      validMethodsArray, validMethodsArray + sizeof(validMethodsArray) /
                                                 sizeof(validMethodsArray[0]));

  if (validMethods.find(method_) == validMethods.end())
    return 501;

  for (std::string::const_iterator it = method_.begin(); it != method_.end();
       ++it) {
...
void HttpRequest::extractPathQueryFragment(size_t pathStart) {
  ...
  if (queryStart != std::string::npos) {
    if (fragmentStart != std::string::npos && fragmentStart > queryStart) {
      ...
      query_ =
          decodedUri.substr(queryStart + 1, fragmentStart - (queryStart + 1));
...
bool HttpRequest::isValidScheme(const std::string &scheme) {
  ...
  const std::string validSchemeChars =
...
int HttpRequest::isValidAuthority(const std::string &authority) {
  ...
  std::string userinfo =
      userinfoEnd != std::string::npos ? authority.substr(0, userinfoEnd) : "";

  size_t hostStart = userinfoEnd != std::string::npos ? userinfoEnd + 1 : 0;
  size_t portStart = authority.find(':', hostStart);
  std::string host = portStart != std::string::npos
                         ? authority.substr(hostStart, portStart - hostStart)
                         : authority.substr(hostStart);

  std::string port =
      (portStart != std::string::npos && portStart + 1 < authority.length())
          ? authority.substr(portStart + 1)
          : "";

  const std::string validAuthorityChars =
      "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-._~:";
  for (size_t i = 0; i < userinfo.length(); ++i) {
    if (!isUnreserved(userinfo[i]) && !isSubDelim(userinfo[i]) &&
        userinfo[i] != ':') {
      return 401;
    }
  }
...
bool HttpRequest::isValidPath(const std::string &path) {
  const std::string validPathChars =
      "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-._~:/"
      "?#[]@!$&'()*+,;=";
...
bool HttpRequest::isValidQuery(const std::string &query) {
  if (query.empty())
    return true;
  const std::string validQueryChars =
      "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-._~:/"
      "?#[]@!$&'()*+,;=";
...
bool HttpRequest::isValidFragment(const std::string &fragment) {
  if (fragment.empty())
    return true;
  const std::string validFragmentChars =
      "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-._~:/"
      "?#[]@!$&'()*+,;=";
...
std::string HttpRequest::getUriSuffix() { return uri_suffix_; }
...
bool HttpRequest::isValidIPv6(const std::string &ipv6) {
  ...
  if (numColons > 7 || ipv6.find("::") != std::string::npos ||
      (numColons == 7 && ipv6[ipv6.length() - 1] == ':'))
    return false;
